...
import gov.nist.ngac.pm.core.common.exception.AssignmentCausesLoopException;
import gov.nist.ngac.pm.core.common.exception.CannotDeleteAdminPolicyConfigException;
import gov.nist.ngac.pm.core.common.exception.DisconnectedNodeException;
import gov.nist.ngac.pm.core.common.exception.InvalidAssignmentException;
import gov.nist.ngac.pm.core.common.exception.InvalidAssociationException;
import gov.nist.ngac.pm.core.common.exception.NodeDoesNotExistException;
import gov.nist.ngac.pm.core.common.exception.NodeHasAscendantsException;
import gov.nist.ngac.pm.core.common.exception.NodeIdExistsException;
import gov.nist.ngac.pm.core.common.exception.NodeNameExistsException;
import gov.nist.ngac.pm.core.common.exception.NodeReferencedInObligationException;
import gov.nist.ngac.pm.core.common.exception.NodeReferencedInProhibitionException;
...
public class GraphModifier extends Modifier implements GraphModification {
    ...
    public long createPolicyClass(String name) throws PMException {
        ...
        long id = generateUniqueId(name, PC);
    ...
    /**
     * Retry id generation on collision.
     *
     * @param name The node name being created, passed through to the generator on retry.
     * @param type The node type being created, passed through to the generator on retry.
     * @return An id that does not collide with an existing node.
     * @throws NodeIdExistsException If every attempt collides with an existing node.
     */
    private long generateUniqueId(String name, NodeType type) throws PMException {
        long id = idGenerator.generateId(name, PC);

        for (int attempt = 1; policyStore.graph().nodeExists(id); attempt++) {
            if (attempt >= MAX_ID_GENERATION_ATTEMPTS) {
                throw new NodeIdExistsException(id);
            }
            id = idGenerator.generateId(name, type);
        }
        return id;
    }
    ...
    throws PMException {
        ...
        final long uniqueId = generateUniqueId(name, type);
        ...
        return runTx(() -> {
            policyStore.graph().createNode(uniqueId, name, type);

            for (long desc : descendants) {
                policyStore.graph().createAssignment(uniqueId, desc);
            }
