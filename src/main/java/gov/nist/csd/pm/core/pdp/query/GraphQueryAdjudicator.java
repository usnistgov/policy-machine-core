...
public class GraphQueryAdjudicator extends Adjudicator implements GraphQuery {
    ...
    public boolean nodeExists(String name) throws PMException {
        try {
            ...
            // check user has permissions on the node
            check(userCtx, new IdTargetContext(node.getId()));
    ...
    public Collection<Long> getPolicyClasses() throws PMException {
        check(userCtx, new IdTargetContext(AdminPolicyNode.PM_ADMIN_POLICY_CLASSES.nodeId()),
    ...
    public Collection<Long> getAdjacentDescendants(long nodeId) throws PMException {
        check(userCtx, new IdTargetContext(nodeId), AdminAccessRight.ADMIN_GRAPH_ASSIGNMENT_LIST);
    ...
    public Subgraph getAscendantSubgraph(long nodeId) throws PMException {
        check(userCtx, new IdTargetContext(nodeId), AdminAccessRight.ADMIN_GRAPH_SUBGRAPH_LIST);
    ...
    public boolean isAscendant(long ascendantId, long descendantId) throws PMException {
        check(userCtx, new IdTargetContext(ascendantId), AdminAccessRight.ADMIN_GRAPH_ASSIGNMENT_LIST);
        check(userCtx, new IdTargetContext(descendantId), AdminAccessRight.ADMIN_GRAPH_ASSIGNMENT_LIST);
    ...
    private List<Association> getAssociations(Collection<Association> associations) throws PMException {
        ...
        for (Association association : associations) {
            try {
                check(userCtx, new IdTargetContext(association.source()), AdminAccessRight.ADMIN_GRAPH_ASSOCIATION_LIST);
                check(userCtx, new IdTargetContext(association.target()), AdminAccessRight.ADMIN_GRAPH_ASSOCIATION_LIST);
    ...
    private Collection<Long> filterNodes(Collection<Long> nodes, AdminAccessRight ar) throws PMException {
        try {
            nodes.removeIf(id -> {
                try {
                    check(userCtx, new IdTargetContext(id), ar);
                    return false;
                } catch (UnauthorizedException e) {
                    return true;
                } catch (PMException e) {
