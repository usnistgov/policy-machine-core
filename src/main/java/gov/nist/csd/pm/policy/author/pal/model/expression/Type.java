...
public class Type implements Serializable {
    ...
    public static Type toType(PALParser.VarTypeContext varTypeContext) {
        Type type = null;
        if (varTypeContext instanceof PALParser.StringTypeContext) {
            type = Type.string();
        } else if (varTypeContext instanceof PALParser.BooleanTypeContext) {
            type = Type.bool();
        } else if (varTypeContext instanceof PALParser.ArrayVarTypeContext arrayVarTypeCtx) {
            type = Type.array(toType(arrayVarTypeCtx.arrayType().varType()));
        } else if (varTypeContext instanceof PALParser.MapVarTypeContext mapVarTypeContext) {
            type = Type.map(
                    toType(mapVarTypeContext.mapType().keyType),
                    toType(mapVarTypeContext.mapType().valueType)
            );
        } else if (varTypeContext instanceof PALParser.AnyTypeContext) {
            type = Type.any();
        }
