...
CREATE FUNCTION `get_node_id`(attr_in varchar(50)) RETURNS int
BEGIN
...
CREATE FUNCTION `assignment_allowed`(child_name varchar(50), parent_name varchar(50)) RETURNS tinyint(1)
BEGIN
...
-- If child is not a child, nor a parent of the 'parent' node, return true - assignment allowed
    ...
    with recursive cte (start_node_id, end_node_id)
                       as (select a.start_node_id, a.end_node_id
                           from assignment a
